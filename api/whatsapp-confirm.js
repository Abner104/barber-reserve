...
export default async function handler(req, res) {
  ...
  const { barberId, code } = b || {};
  if (!barberId || !code) { res.status(400).json({ error: "barberId y code requeridos" }); return; }

  const r    = await fetch(SUPABASE_URL + "/rest/v1/whatsapp_verifications?barber_id=eq." + barberId + "&verified=eq.false&select=*&order=created_at.desc&limit=1", { headers: dbH });
  ...
  if (row.code !== String(code).trim()) { res.status(400).json({ error: "Código incorrecto" }); return; }

  await fetch(SUPABASE_URL + "/rest/v1/whatsapp_verifications?id=eq." + row.id, {
    method: "PATCH", headers: dbH, body: JSON.stringify({ verified: true }),
  });
  await fetch(SUPABASE_URL + "/rest/v1/barbers?id=eq." + barberId, {
    method: "PATCH", headers: dbH, body: JSON.stringify({ whatsapp_number: row.phone, whatsapp_verified: true }),
  });
